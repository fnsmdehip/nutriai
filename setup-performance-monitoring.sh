#!/bin/bash
# setup-performance-monitoring.sh - Sets up monitoring for API performance

set -e

echo "=== Setting up performance monitoring locally ==="

# Create monitoring script directory if it doesn't exist
mkdir -p ./scripts
mkdir -p ./logs/performance-metrics

# Create the performance monitoring script
cat > ./scripts/monitor-api-performance.sh << 'EOF'
#!/bin/bash
...
check_container_resources() {
    ...
    if docker ps | grep -q "riskoptimize_api_1"; then
        ...
        local container_stats=$(docker stats riskoptimize_api_1 --no-stream --format "{{.CPUPerc}}|{{.MemPerc}}|{{.NetIO}}|{{.BlockIO}}")
        ...
        if [ -n "$container_stats" ]; then
            ...
            log "Failed to get container stats for riskoptimize_api_1"
        fi
    else
        log "Container riskoptimize_api_1 is not running"
    fi
}
...
chmod +x ./scripts/monitor-api-performance.sh

# Create stats collection script 
cat > ./scripts/collect-daily-stats.sh << 'EOF'
#!/bin/bash
...
if [ -f "$LOG_DIR/api-performance.log" ] && grep -q "Response time:" "$LOG_DIR/api-performance.log"; then
    ...
    AVG_RESPONSE=$(grep "Response time:" "$LOG_DIR/api-performance.log" | grep -oP '\d+(?=ms)' | awk '{ total += $1; count++ } END { if (count > 0) print total/count; else print "N/A" }')
    echo "Average response time: ${AVG_RESPONSE}ms" >> "$STATS_FILE"

    # Get 95th percentile
    PERCENTILE_95=$(grep "Response time:" "$LOG_DIR/api-performance.log" | grep -oP '\d+(?=ms)' | sort -n | awk 'BEGIN{c=0} {a[c]=$1; c++} END{print a[int(c*0.95)]}')
...
if [ -f "$LOG_DIR/api-performance.log" ] && grep -q "CPU:" "$LOG_DIR/api-performance.log"; then
    # CPU Average
    CPU_AVG=$(grep "CPU:" "$LOG_DIR/api-performance.log" | grep -oP '(?<=CPU: )[^,]+' | sed 's/%//' | awk '{ total += $1; count++ } END { if (count > 0) print total/count; else print "N/A" }')
    echo "Average CPU usage: ${CPU_AVG}%" >> "$STATS_FILE"

    # Memory Average
    MEM_AVG=$(grep "Memory:" "$LOG_DIR/api-performance.log" | grep -oP '(?<=Memory: )[^,]+' | sed 's/%//' | awk '{ total += $1; count++ } END { if (count > 0) print total/count; else print "N/A" }')
...
chmod +x ./scripts/collect-daily-stats.sh

# Create empty files to avoid initially missing file errors
touch ./logs/performance-metrics/api-performance.log
touch ./logs/performance-metrics/alerts.log
touch ./logs/performance-metrics/historical_stats.log

echo "Performance monitoring setup complete."
echo "Logs will be stored in ./logs/performance-metrics/"
echo ""
echo "To run the monitoring manually:"
echo "  ./scripts/monitor-api-performance.sh"
echo ""
echo "To collect daily statistics manually:"
echo "  ./scripts/collect-daily-stats.sh"
echo ""
echo "For production use, consider setting up cron jobs:"
echo "  - Run performance check every 5 minutes: */5 * * * * /path/to/scripts/monitor-api-performance.sh"
echo "  - Generate daily stats at midnight: 0 0 * * * /path/to/scripts/collect-daily-stats.sh"
