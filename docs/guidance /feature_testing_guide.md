...
**CRITICAL REQUIREMENT**: ALL agents MUST scan docs/hyper rat soft engin.rtf BEFORE working on any feature testing or responding to any testing-related query. This document contains the non-negotiable methodological framework required for all project work.

**TESTING METHODOLOGY REQUIREMENTS**:

- Apply hyper-rational deconstruction to all testing processes
- Challenge assumptions in feature specifications
- Identify edge cases through first-principles analysis
- Focus on objective, measurable test criteria
- Eliminate subjectivity and fuzzy acceptance criteria
- Document specific, concrete pass/fail conditions

Any agent failing to apply the methodology in docs/hyper rat soft engin.rtf will produce substandard testing protocols that WILL be rejected.

## COMMUNICATION PROTOCOL - MANDATORY

**CRITICAL: READ AND FOLLOW THESE INSTRUCTIONS**

All communication during feature testing MUST follow the hyper-rational protocol defined in `hyper-rat-soft-engin.rtf`. Every single response must be structured as follows:
...
This communication structure is non-negotiable. It ensures efficient problem diagnosis and resolution with zero tolerance for excuses or rationalization.
